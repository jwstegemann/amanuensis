...
object StoryActor {
  ...
  val retrieveStoryQueryString = """
    MATCH (s:Story {id: {id}})<-[:canRead|:canWrite|:canGrant*1..5]-(:User {login: {login}})
  ...
  val retrieveOutSlotQueryString = """
    MATCH (s:Story)-[r:Slot]->()<-[:canRead|:canWrite|:canGrant*1..5]-(:User {login: {login}})
  ...
  val removeStoryQueryString = """
    MATCH (s:Story {id: {id}})<-[:canWrite|:canGrant*1..5]-(:User {login: {login}})
    WITH s.id as id, s
    OPTIONAL MATCH s-[r]-() 
    DELETE r,s
    RETURN id
